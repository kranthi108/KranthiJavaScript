                    ...
                    <tr> -->
                      ...
                      <?php
                         ...
                         //in your case, it would echo whatever $row['Report ID'] is equivalent to.
                              ...
                              if($st==0)
                              {
                                echo "<center><br><br>successfully Added<br>";
                                $sql1="select * from aoi where uname='$username'";
                                $result1=mysqli_query($conn,$sql1);
                                echo "<h3>Area Of Interests</h3><br>";
                                while($row=mysqli_fetch_assoc($result1))
                                {
                                  $area=$row["area"];
                                  echo "<style='text-align:left;'>$area<br>";
                                }
                              }
                            }
                            if($option=='area_of_interest')
                            {
                              $username=$_SESSION["user"];
                                echo "<center><h3>List of fields you may show interests</h3></center>";
                                $sql="select area from areaofinterest where area not IN(select area from aoi where uname='$username');";
                                $result=mysqli_query($conn,$sql);
                                if($result)
                                {
                                 # echo "<table><tr><td>";
                                  echo "<form action='aoi.php' method='get'>";
                                  while($row=mysqli_fetch_assoc($result))
                                  {
                                    $area=$row["area"];
                                    #echo $area;
                                    
                                    echo "<input type='checkbox' name='check_list[]' value='$area' > $area<br><br><br>";

                                  }
                                  #echo "</td></tr>";
                                  echo "<input type='submit' class='btn btn-default' value='Add to the list' name='option'></form>";
                                }

                                echo "<center><h3>Previously in your list</h3></center><br>";
                                $sql1="select * from aoi where uname='$username'";
                                $result1=mysqli_query($conn,$sql1);
